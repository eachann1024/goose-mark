...
let bookmarkPersistenceWritable = false
...
const queueBookmarkPersistJob = (state: BookmarkStore, force = false): boolean => {
  if (!bookmarkPersistenceWritable || !isUToolsDbAvailable()) return false
  if (
    !force &&
...
export const initializeBookmarkStorePersistence = async (): Promise<void> => {
  ...
  try {
    ...
    useBookmarkStore.getState().setReadOnly(true)
    return
  }
  if (persisted) {
  ...
  if (!persisted) {
    baseBookmarkSnapshot = pickBookmarkSnapshot(useBookmarkStore.getState())
  }
  lastPersistedBookmarkState = serializeBookmarkData(baseBookmarkSnapshot!)
  bookmarkPersistenceWritable = true
