...
const initStars = () => {
  ...
  for (let index = 0; index < CORE_STAR_COUNT; index++) {
    stars.push({
      ...
      baseOpacity: Math.random() * (STAR_BASE_OPACITY_MAX - STAR_BASE_OPACITY_MIN) + STAR_BASE_OPACITY_MIN,
      opacity: 0,
...
<template>
  <div class="fixed inset-0 z-0 overflow-hidden pointer-events-none select-none">
    <div class="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#0d1b2a] via-[#020204] to-[#000000] opacity-80" />
