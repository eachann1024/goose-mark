...
import { ArrowUpDown, Check, LayoutGrid, List, Plus, StretchHorizontal } from 'lucide-react'
 ...
 * 右侧一排控件：排序下拉（默认「最近使用」）+ 视图切换（列表/网格/卡片）+「新建」珊瑚按钮。
...
export interface BookmarkListHeaderProps {
  ...
  sortEnabled?: boolean
  /** 是否展示排序下拉（仅「全部书签」视图展示，其余隐藏） */
  showSort?: boolean
  /** @deprecated 标签筛选功能已下线，保留以兼容调用方 */
  tagOptions?: TagFilterOption[]
  /** @deprecated 标签筛选功能已下线 */
  selectedTags?: string[]
  /** @deprecated 标签筛选功能已下线 */
...
export function BookmarkListHeader({
  ...
  return (
    <div className="shrink-0 flex items-center gap-3 px-4 h-[52px] border-b border-border/30 select-none">
      {/* 左侧：视图名（衬线大标题）+ 项数 */}
      <div className="min-w-0 flex items-baseline gap-[7px] whitespace-nowrap">
        <h1 className="text-[15px] leading-none font-semibold tracking-[-0.01em] text-foreground truncate">
          {title}
        </h1>
        <span className="shrink-0 text-[12px] font-mono text-muted-foreground/60 tabular-nums">{count} 项</span>
      </div>
      ...
      <div ref={sortRef} className="relative">
        <button
          ...
          className={cn(
            'h-[30px] inline-flex items-center gap-[5px] pl-2.5 pr-2 rounded-lg border border-border bg-background/60 text-[12.5px] transition-colors',
      ...
      <div className="flex items-center gap-0.5 bg-muted/60 rounded-lg p-0.5">
        {VIEW_MODES.map(({ mode, icon: Icon, label }) => (
          <button
            ...
            type="button"
            className={cn(
              'h-[26px] w-7 flex items-center justify-center rounded-md transition-colors',
              viewMode === mode
      ...
      <button
        type="button"
        className="h-[30px] inline-flex items-center gap-[5px] pl-2.5 pr-3 rounded-lg bg-primary text-primary-foreground text-[12.5px] font-semibold shadow-sm hover:brightness-105 active:brightness-95 transition-all"
        onClick={onCreate}
        title="新建书签"
      >
