...
## UI 收尾闸门（报完成前强制，禁止跳过）
改完 UI 后，**报完成前必须**按下列结构化格式自评，而不是一句"已完成/效果不错"收尾——一句话总结给了蒙混空间，逐条勾叉才逼自己真的看：
1. 浅色 + 深色**双模式各截一张图**，贴给用户。
2. 对照 skill `用户UI偏好与验收` 的「验收 10 条」**逐条写明 ✅/❌**（命中的项才写，每条附一句依据，不许只写"OK"）。
3. 有任意 ❌ → 必须改完重新自评，**全 ✅ 才能报完成**。
4. 自评心态是**对抗式找茬**（"哪里还不够好"），不是自我确认（"看起来能用"）。
